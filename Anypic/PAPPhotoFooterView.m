...
- (void)setPhoto:(PFObject *)object
{
    ...
    NSString *content = [photo objectForKey:kPAPPhotoDescriptionKey];
    ...
    if (price) {
        ...
        priceLabel.text = ([[photo objectForKey:kPAPPhotoIsSoldKey] isEqualToString:@"1"] ? @"NowNear" : [NSString stringWithFormat:@"$%@", [photo objectForKey:kPAPPhotoPriceKey]]);
    }
    else {
        priceLabel.text = @"";
    }

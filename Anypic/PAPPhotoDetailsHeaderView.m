...
- (void)setBuyButtonAndPriceLabel {
    ...
    if (price) {
        priceLabel.text = ([[photo objectForKey:kPAPPhotoIsSoldKey]
                            isEqualToString:@"1"] ? @"NowNear" : [NSString stringWithFormat:@"$%@ | Buy Now", [photo objectForKey:kPAPPhotoPriceKey]]);
    }
    else {
        priceLabel.text = @"";
    }
...
//    self.likeButton.layer.borderColor = [UIColor redColor].CGColor;
    ...
    if ([photo objectForKey:kPAPPhotoTitleKey] != nil) {
        titleTextView.text = [NSString stringWithFormat:@"%@", [photo objectForKey:kPAPPhotoTitleKey]];
    }
    else {
        ...
        NSString * shoeName = [photo objectForKey:kPAPPhotoDescriptionKey];

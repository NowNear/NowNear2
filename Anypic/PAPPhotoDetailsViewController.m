...
- (void) viewWillAppear:(BOOL)animated {
    ...
    [self.headerView setBuyButtonAndPriceLabel];
}
...
- (void)viewDidLoad {
    ...
    [super viewDidLoad];
    self.navigationItem.title = [photo objectForKey:kPAPPhotoDescriptionKey];
...
//    self.tableView.backgroundView = texturedBackgroundView;
    ...
    // Set table header
    self.headerView = [[NSBundle mainBundle] loadNibNamed:@"PAPPhotoDetailsHeaderView" owner:nil options:nil][0];
    self.headerView.frame = [PAPPhotoDetailsHeaderView rectForViewWithDescription:[photo objectForKey:kPAPPhotoDescriptionKey]];
    self.headerView.delegate = self;
    self.headerView.photo = self.photo;
    self.headerView.photographer = [self.photo objectForKey:kPAPPhotoUserKey];
    [self.headerView createView];
    ...
    //UIBarButtonItem *backBt = [[UIBarButtonItem alloc]initWithTitle:@"< Back" style:UIBarButtonItemStyleBordered target:self action:@selector(backButtonACtion:)];
    ...
    if (trimmedComment.length != 0 && [self.photo objectForKey:kPAPPhotoUserKey]) {
        ...
        [comment saveEventually:^(BOOL succeeded, NSError *error) {
            ...
            if (error && error.code == kPFErrorObjectNotFound) {
                ...
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Could not post comment", nil) message:NSLocalizedString(@"This item is no longer available", nil) delegate:nil cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
                [alert show];
                [self.navigationController popViewControllerAnimated:YES];
            }
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (actionSheet.tag == MainActionSheetTag)
    {
        if ([actionSheet destructiveButtonIndex] == buttonIndex) {
            // prompt to delete
            UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:NSLocalizedString(@"Are you sure you want to delete this listing?", nil) delegate:self cancelButtonTitle:NSLocalizedString(@"Cancel", nil) destructiveButtonTitle:NSLocalizedString(@"Yes, delete this listing", nil) otherButtonTitles:nil];
         ...
         // if share button pressed then show share dialog
        else if(buttonIndex == 1){
            [self activityButtonAction:actionSheet];
        }
        // If 'Mark as Sold' button pressed then change the 'isSold' column to 1.
        else if(buttonIndex == 2){
            [photo setObject:@"1" forKey:kPAPPhotoIsSoldKey];
            [photo saveInBackground];
        }
        // If 'Mark as Featured' button pressed then change the 'isFeatured' column to Yes.
        else if(buttonIndex == 2){
            [photo setObject:@"Yes" forKey:kPAPPhotoIsFeaturedKey];
            [photo saveInBackground];
        }
    }
...
- (void)photoDetailsView:(PAPPhotoDetailsHeaderView *)photoDetailsView didTapBuyButton:(UIButton *)button
{
    ...
    [PFCloud callFunctionInBackground:kRLPerformPhotoReservationFunction withParameters:@{kRLPhotoIdParameterKey:self.photo.objectId} block:^(id object, NSError *error) {
        
        if (!error)
        {
            // Open summary
            RLCheckoutOverviewViewController *checkoutOverviewViewController = [[RLCheckoutOverviewViewController alloc] initWithNibName:@"RLCheckoutOverviewViewController" bundle:nil];
            checkoutOverviewViewController.photoObject = self.photo;
            
            UINavigationController *navigationController = [[UINavigationController alloc] initWithRootViewController:checkoutOverviewViewController];
        ...
        else
        {
            ...
            if (error.code == kPFErrorTimeout)
            {
                errorMessage = @"Check your Internet connection";
            }
            else
            {
                errorMessage = @"Other user is trying to buy this item, please check again later";
            }
            ...
            [[[UIAlertView alloc] initWithTitle:@"Error"
             ...
             NSLog(@"Could not perform photo reservation: %@", error.description);
        }
...
- (void)photoDetailsView:(PAPPhotoDetailsHeaderView *)photoDetailsView didTapBuyOnPhotoButton:(UIButton*)button // photo:(PFObject *)photo
{
...
- (void)actionButtonAction:(id)sender {
    ...
    actionSheet.destructiveButtonIndex = [actionSheet addButtonWithTitle:NSLocalizedString(@"Delete Item", nil)];
    if (NSClassFromString(@"UIActivityViewController")) {
        [actionSheet addButtonWithTitle:NSLocalizedString(@"Share Item", nil)];
        [actionSheet addButtonWithTitle:NSLocalizedString(@"Mark as Sold", nil)];
